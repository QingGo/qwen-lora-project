...
`train_qwen_lora.py` 通过参数支持四种训练模式：

| 模式 | 参数 | 显存 (7B) | 说明 |
|------|-------|-----------|------|
| LoRA (bf16) | *(默认)* | ~18 GB | 最优质量，标准模式 |
| LoRA + DeepSpeed | `--ds_stage 2` | ~18 GB | 单卡可用 |
| QLoRA (4-bit) | `--qlora` | ~9 GB | 省一半显存，质量略有损失 |
| QLoRA + DeepSpeed | `--qlora --ds_stage 2` | ~9 GB | 最大显存节省 |

所有 DeepSpeed 模式需设置环境变量: `RANK=0 WORLD_SIZE=1 LOCAL_RANK=0 MASTER_ADDR=localhost MASTER_PORT=29500`

```bash
# 纯 LoRA（默认）
uv run python train_qwen_lora.py --data_path ./data/text2sql/clean/train.jsonl

# LoRA + DeepSpeed ZeRO-2（单卡）
...
# QLoRA + DeepSpeed ZeRO-2（最大显存节省）
RANK=0 WORLD_SIZE=1 LOCAL_RANK=0 MASTER_ADDR=localhost MASTER_PORT=29500 \
  uv run python train_qwen_lora.py --data_path ./data/text2sql/clean/train.jsonl --qlora --ds_stage 2

# DeepSpeed ZeRO-3（需 2 卡以上，单卡 OOM）
# bash scripts/launch_multi.sh 4 3   # 4 卡
# 注：使用 --deepspeed_config configs/ds_zero3.json 可完全控制配置
```
...
对 L5 adapter 权重的详细分析（r=16, alpha=32, 1800步, lr=2e-4 cosine）：
...
输出投影变化最大，K/V 投影变化最小——输出投影决定最终 token 分布，符合直觉。
...
|--------|:--------:|:------:|
| 浅层 (0-4) | 1.80 | 4.18% |
| 中层 (12-16) | 1.57 | **3.73%** ← 最小 |
| 深层 (23-27) | 2.38 | **5.20%** ← 最大 |

U 形分布：浅层和深层变化大，中层变化小。最大贡献为 layer_24/o_proj，||delta||=4.86（占总变化的 5.3%）。

**理论 vs 实际：**

Adam 优化器，假设梯度方向一致的随机游走模型：
```
|Δ|_F(理论) = lr_peak × √(N × T)
            = 2e-4 × √(10M × 1800) ≈ 26.8
|Δ|_F(实际) = 21.18  →  比率 0.79x
```

剩余 21% 差距归因于：(1) cosine 调度使平均 lr 低于峰值，(2) 梯度方向并非完全随机/一致，(3) weight decay 正则化抵消。
...
17. **LoRA 权重变化轻量且结构化**：Adapter 平均修改原权重 ~4%。变化集中在 o_proj（输出投影），呈 U 形层分布（浅深 > 中层）。Adam 优化器有效步长约等于 lr，远大于 SGD 的 lr × gradient。随机游走理论模型与实际匹配度 0.79x。

## 未来方向

**LoRA Text2SQL 优化已暂停。** 以下方向供后续工作参考：

1. **Schema 注入**：prompt 中标注 `table.column` 前缀 + 显式 FK 关系。目标：解决列名幻觉（18/28 LoRA 失败）。

2. **Best-of-N + 执行过滤**：生成 5-10 条 SQL 候选，取第一条可执行的。低成本的 schema 错误概率命中策略。

3. **负样本训练**：合成错误（随机替换同 schema 列/表名）加入训练，针对性纠正 schema 映射。

4. **更多数据 + 更长上下文**：10K+ SQaLe 样本 + max_length=4096。当前规模下边际收益预计有限。

5. **DPO / 对比学习**：(问题, 正确SQL, 模型错误SQL) 三元组进行定向校正。

## Function Calling

Qwen2.5-7B-Instruct 原生支持 Hermes 风格的 `<tool_call>` 函数调用，**无需微调**。

**Demo**：`function_calling/demo.py` — 定义 3 个工具（天气、计算器、数据库搜索），运行多轮对话与工具执行循环。
...
| 计算器 | 正确表达式 |
| 数据库搜索 | 正确查询 + 格式化输出 |
| 发送邮件（含参数推理） | 自动生成缺失参数（收件人、正文） |
| 创建会议（自然语言时间） | 提取时间信息 |
| 无需工具（"今天星期几"） | 正确跳过工具，直接回答 |
| 并行多工具（邮件 + 会议） | 一次输出 2 个 tool_call |

**关键发现：**
- 基座模型原生理解 `<tools>` → `<tool_call>` → `<tool_response>` 对话流程
- 参数提取在模糊/缺失输入时仍有效（模型补全合理默认值）
...
### GPU 基准测试：BF16 GGUF vs F16 GGUF vs Q4_K_M（RTX 4090，CUDA）

| 指标 | BF16 GGUF | F16 GGUF | Q4_K_M GGUF |
|------|:------:|:------:|:---------:|
| 模型大小 | 15.0 GB | 15.0 GB | **4.4 GB** |
| GPU 显存 | 15.9 GB | 15.9 GB | **6.6 GB** |
| Prompt（短）| 223 t/s | 370 t/s | **999 t/s** |
| Prompt（Text2SQL）| 410 t/s | 587 t/s | **2,511 t/s** |
| 生成（短）| 68 t/s | 78 t/s | **160 t/s** |
| 生成（Text2SQL）| 67 t/s | 66 t/s | **167 t/s** |
| Text2SQL 质量 | 正确 | 正确 | 正确（完全相同）|
| 精度 | 原生 BF16 | BF16→F16 转换 | BF16 量化 |

**关键发现：**
- **BF16 vs F16**：两者模型大小相同（15GB），GPU 显存相同（~15.9GB）。RTX 4090（Ada Lovelace）上 FP16 和 BF16 均有原生 Tensor Core 支持，性能相当。
- **Q4_K_M vs BF16**：快 2-7 倍，小 3.25x，省显存 2.4x，质量无损。
- **Q4_K_M 是部署最优选** — 最快、最小、质量无损。
...
# 2. 编译 llama.cpp（CUDA 支持，RTX 4090）
...
- deepspeed + mpi4py（可选，`--ds_stage 2` 单卡可用）
- bitsandbytes（可选，`--qlora` 4-bit 量化训练）
